...
//  Notenbuch
//
//  Created by Tom Kumschier on 01.07.15.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "me.angelicornis.Notenbuch" in the application's documents Application Support directory.
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("Notenbuch", withExtension: "momd")!
    ...
    static func minMaxIntegerValueForEntitiy(entityName: String, attributeName: String, minimum: Bool, predicate: NSPredicate = NSPredicate(value: true)) ->Int {
        ...
        do {
            let object = try context.executeFetchRequest(request).first as? NSManagedObject
    ...
    static func move(entityName:String, orderAttributeName:String, source: NSManagedObject, toDestination destination: NSManagedObject) {
        if let sourceOrder = source.valueForKey(orderAttributeName)?.integerValue, destinationOrder = destination.valueForKey(orderAttributeName)?.integerValue {
            let request = NSFetchRequest(entityName: entityName)
            
            if sourceOrder < destinationOrder {
                request.sortDescriptors = [NSSortDescriptor(key: kFachart, ascending: true) ,NSSortDescriptor(key: orderAttributeName, ascending: true)]
            ...
            do {
                ...
                print("Vehler beim Speichern beim Verschieben")
            }
        }
    }
}
